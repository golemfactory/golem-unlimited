...
impl Platforms {
    pub fn try_new() -> Result<Self, Error> {
        ...
        let ret =
            unsafe { clGetPlatformIDs(0, ptr::null_mut(), &mut num_platforms as *mut cl_uint) };
        if ret != CL_SUCCESS {
        ...
        let ret = unsafe {
            clGetPlatformIDs(
                platforms.capacity() as u32,
                platforms.as_mut().as_mut_ptr(),
                &mut num_platforms as *mut cl_uint,
            )
        };
        if ret != CL_SUCCESS {
            return Err(Error(ret));
...
#[inline(always)]
unsafe fn extract_string<F: Fn(usize, *mut c_void, *mut usize) -> cl_int>(
    f: F,
) -> Result<String, Error> {
    let mut size = 0;
...
where
    F: Fn(cl_uint, *mut A::Item, *mut cl_uint) -> cl_int,
    A: Array,
{
...
impl Platform {
    pub fn devices(&self) -> Result<Devices, Error> {
        let devices = unsafe {
            extract_vec(|num_entries, devices, num_devices| {
                clGetDeviceIDs(
                    self.0,
