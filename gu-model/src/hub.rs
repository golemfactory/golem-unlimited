...
mod test {
    ...
    fn test_serialize() {
        let hub_info = HubInfo {
            node_id: "0xf6140a03926b0801cd891d2d128ebd8dffbda252".parse().unwrap(),
            ...
            caps: (vec![
                ("gu.env.hd".to_string(), Capability {
                    version: "0.1.0".parse().unwrap(),
                    props: Map::default()
                }),
                ("gu.env.docker".to_string(), Capability {
                    version: "0.1.0".parse().unwrap(),
                    props: Map::default()
                }),
                ("gu.session.config".to_string(), Capability {
                        version: "0.1.0".parse().unwrap(),
                        props: Map::default()
                }),
                ("gu.session.blob".to_string(), Capability {
                    version: "0.2.0".parse().unwrap(),
                    props: Map::default()
                })
            ]).into_iter().collect()
