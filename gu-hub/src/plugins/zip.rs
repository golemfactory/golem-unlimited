...
pub fn validate_and_load_metadata(
    path: &Path,
    gu_version: Version,
    ...
    if !metadata.proper_version(&gu_version) {
        return Err(format!("Too low gu-app version ({})", gu_version));
    }
...
/// Loads zip archive from `zip_path`, containing `app_name` plugin inside,
/// into hashmap
pub fn load_archive(
    zip_path: &Path,
    app_name: &String,
    ...
    for i in 0..archive.len() {
        ...
        match out_path.strip_prefix(app_name) {
            Ok(path) => {
                ...
                map.insert(path.to_path_buf(), buf);
            }
            _ => (),
        }
    }
