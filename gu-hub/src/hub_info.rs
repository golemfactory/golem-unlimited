...
use gu_model::{HubInfo, BuildInfo, Version, Map};
use actix_web::Json;
...
impl InfoModule {
    ...
    fn create_info(&self) -> HubInfo {
        let node_id = {
            self.ref_node_id.read().unwrap().clone().unwrap()
        };
...
impl Module for InfoModule {
    ...
    fn decorate_webapp<S: 'static>(&self, app: actix_web::App<S>) -> actix_web::App<S> {
       ...
       app.resource("/info", move |r| r.get().with(move |_ : ()|
           Json(info.clone())
       ))
    }
}
...
pub fn module() -> impl Module {
    InfoModule { ref_node_id: RwLock::new(None) }
}