...
use actix::prelude::*;
use actix_web::{HttpRequest, HttpResponse, HttpMessage, Responder, App, Json, AsyncResponder};
use actix_web;
...
impl Handler<AddProvider> for StatusManager {
    ...
    fn handle(&mut self, msg: AddProvider, ctx: &mut Self::Context) -> <Self as Handler<AddProvider>>::Result {
...
impl Handler<ListEnvStatus> for StatusManager {
    ...
    fn handle(&mut self, msg : ListEnvStatus, ctx : &mut Self::Context) -> Self::Result {
        ActorResponse::async(
        future::join_all(
        self.providers.clone().into_iter().map(move |(env_name, env_addr)| {
            let name = env_name.to_string();
            env_addr.send(GetEnvStatus).and_then(move |s| Ok((name, s)))
        })).and_then(|envs| -> Result<BTreeMap<String, EnvStatus>, actix::MailboxError> {
