...
impl NodeId {
    ...
    #[inline(always)]
    fn with_hex<F, R>(&self, f : F) -> R where F : FnOnce(&str) -> R {
...
impl<'de> de::Visitor<'de> for NodeIdVisit {
    ...
    fn visit_str<E>(self, v: &str) -> Result<<Self as de::Visitor<'de>>::Value, E> where
        E: de::Error, {
    ...
    fn visit_bytes<E>(self, v: &[u8]) -> Result<<Self as de::Visitor<'de>>::Value, E> where
        E: de::Error, {
        if v.len() == 32 {
